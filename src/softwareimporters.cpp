...
void SoftwareImporters::importKJots()
{
    ...
    BasketFactory::newBasket(QStringLiteral("kjots"), i18n("From KJots"));
    ...
    for (QStringList::Iterator it = list.begin(); it != list.end(); ++it) { // For each file
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            if (!buf.isNull() && buf.left(9) == "\\NewEntry") {
                ...
                BasketFactory::newBasket(QStringLiteral("kjots"),
                                         QUrl::fromLocalFile(file.fileName()).fileName(),
                                         kjotsBasket);
                ...
                // First create a basket for it:
                BasketFactory::newBasket(QStringLiteral("kjots"), bookTitle, kjotsBasket);
                BasketScene *basket = Global::bnpView->currentBasket();
                basket->load();
...
void SoftwareImporters::importKNotes()
{
    ...
    for (QStringList::Iterator it = list.begin(); it != list.end(); ++it) { // For each file
        ...
        if (file.open(QIODevice::ReadOnly)) {
            ...
            // First create a basket for it:
            BasketFactory::newBasket(QStringLiteral("knotes"), i18n("From KNotes"));
            BasketScene *basket = Global::bnpView->currentBasket();
            basket->load();
...
void SoftwareImporters::importStickyNotes()
{
    ...
    for (QStringList::Iterator it = founds.begin(); it != founds.end(); ++it) { // For each file
        ...
        // First create a basket for it:
        BasketFactory::newBasket(QStringLiteral("gnome"), i18n("From Sticky Notes"));
        BasketScene *basket = Global::bnpView->currentBasket();
        basket->load();
...
void SoftwareImporters::importTomboy()
{
    ...
    BasketScene *basketFromTomboy = nullptr; // Create the basket ONLY if we found at least one note to add!
    ...
    for (int L = 0; L < 2; L++) {
        ...
        for (QStringList::Iterator it = list.begin(); it != list.end(); ++it) { // For each file
            ...
            if (basketFromTomboy == nullptr) {
                // First create a basket for it:
                BasketFactory::newBasket(IconNames::TOMBOY, i18n("From Tomboy"));
            ...
            if (notebook.length() > 0) {
                if (subBuskets.contains(notebook))
                    ...
                    BasketFactory::newBasket(IconNames::TOMBOY, notebook, basketFromTomboy);
...
void SoftwareImporters::importJreepadFile()
{
    ...
    QString fileName = QFileDialog::getOpenFileName(nullptr, QString(), "kfiledialog:///:ImportJreepadFile", "*.xml|XML files");
    if (fileName.isEmpty()) {
        return;
    }
    ...
    BasketFactory::newBasket(QStringLiteral("xml"), doc->documentElement().attribute("title"));
    ...
    while (!elements.isEmpty()) {
        ...
        for (QDomNode n = currentElement.second.firstChild(); !n.isNull(); n = n.nextSibling()) {
            if (n.isText()) {
                ...
                BasketFactory::newBasket(QStringLiteral("xml"), n.toElement().attribute("title"), currentElement.first);
                basket = Global::bnpView->currentBasket();
                basket->load();
                basketList << basket;
                newElement.first = basket;
...
{
    QUrl url = QFileDialog::getOpenFileUrl(nullptr, QString(), QUrl("kfiledialog:///:ImportKnowIt"), "*.kno|KnowIt files\n*|All files");
    if (!url.isEmpty()) {
        QFile file(url.path());
        ...
        BasketFactory::newBasket(QStringLiteral("knowit"), info.baseName());
        basket = Global::bnpView->currentBasket();
        basket->load();
        baskets.push(basket);
                ...
                if (line.startsWith(QLatin1String("\\NewEntry")) || line.startsWith(QLatin1String("\\CurrentEntry")) || stream.atEnd()) {
                    ...
                    if (!name.isEmpty()) {
                        if ((level == 0 && hierarchy == 1) || (hierarchy == 0)) {
                            BasketFactory::newBasket(QStringLiteral("knowit"), name, baskets.top());
                            basket = Global::bnpView->currentBasket();
                            basket->load();
                        }
...
{
    QString fileName = QFileDialog::getOpenFileName(nullptr, QString(), "kfiledialog:///:ImportTuxCards", "*|All files");
    if (fileName.isEmpty())
        return;
    ...
    if (document == nullptr) {
        KMessageBox::error(nullptr, i18n("Can not import that file. It is either corrupted or not a TuxCards file."), i18n("Bad File Format"));
        return;
    }

    QDomElement collection = document->documentElement();
    int remainingHierarchy = (hierarchy == 0 ? 65000 : 3 - hierarchy);
    importTuxCardsNode(collection, /*parentBasket=*/nullptr, /*parentNote=*/nullptr, remainingHierarchy);
}
...
void SoftwareImporters::importTuxCardsNode(const QDomElement &element, BasketScene *parentBasket, Note *parentNote, int remainingHierarchy)
{
    for (QDomNode n = element.firstChild(); !n.isNull(); n = n.nextSibling()) {
        ...
        if (isEncrypted) {
            KMessageBox::information(nullptr, i18n("A note is encrypted. The importer does not yet support encrypted notes. Please remove the encryption with TuxCards and re-import the file."), i18n("Encrypted Notes not Supported Yet"));
            isRichText = true;
        ...
        if (remainingHierarchy > 0) {
            BasketFactory::newBasket(icon, name, parentBasket);
            BasketScene *basket = Global::bnpView->currentBasket();
            basket->load();
            ...
            importTuxCardsNode(e, basket, nullptr, remainingHierarchy - 1);
            finishImport(basket);
        } else {
